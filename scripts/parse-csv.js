...
async function main() {
    ...
    for (const rawFile of rawFiles) {
        ...
        const destinationDir = path.join(__dirname, '..', 'csv', amount)
        ...
        const csvFileName = rawFile.replace(path.extname(rawFile), '.csv');
        const destinationPath = path.join(destinationDir, csvFileName);

        if (fs.existsSync(destinationPath)) {
            console.log(`CSV file ${csvFileName} already exists. Skipping parsing.`);
            continue;
        }

        const sourceStream = fs.createReadStream(sourcePath);
        const drawWinners = await parseRawToRecords(sourceStream);
        const csvContent = toCsv(drawWinners);
        fs.writeFileSync(destinationPath, csvContent);

        console.log(`Wrote CSV file: ${csvFileName} with ${drawWinners.length} records.`);
    }
...
async function parseRawToRecords(readStream) {
    const drawWinners = [];
...
function toCsv(records) {
    const header = 'number,prize';
    const rows = records.map(({ number, prize }) => `${number},${prize}`);
